...
import type { Grado, Materia, EjercicioGenerado } from "@/lib/types/database";
  ...
  try {
    ...
    const getModel = (id: string) => genAI.getGenerativeModel({
      ...
      generationConfig: {
        ...
        // @ts-expect-error - thinkingConfig no está tipado en la versión actual del SDK
    ...
    try {
      ...
      result = await model.generateContent({
        systemInstruction: buildSystemPrompt(grado, materia),
        contents: [{ role: "user", parts: [{ text: buildUserPrompt(tema) }] }],
      });
    } catch (error: any) {
      if (error?.status === 503) {
        console.warn(`⚠️ ${modelId} saturado (503). Reintentando con ${fallbackModelId}...`);
    ...
    return [
      {
        consigna: `Esta es una pregunta simulada sobre ${tema} para ${grado}° grado. ¿Cuál es la respuesta correcta?`,
        opciones: [
        ...
        explicacion: "¡Muy bien! Recordá que esto es un ejercicio simulado porque hubo un error con la API.",
      },
      {
...
export async function generarResumenMensual(
  ...
  try {
    ...
    const prompt = `Actuá como una docente particular experta en psicopedagogía en Argentina. Tu tarea es redactar un "Informe de Seguimiento Pedagógico Mensual" para los padres de ${alumno.nombre} ${alumno.apellido} (${alumno.grado}° grado).
...
- Observaciones de hitos: ${(hitos || []).map((h: any) => h.resumen_ia).slice(0, 5).join(". ") || "Sin hitos específicos registrados"}
